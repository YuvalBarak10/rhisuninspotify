...
def send_welcome(message):
    bot.send_message(message.from_user.id,
                     "שלום.לחץ  /search כדי לחפש שירים בספוטיפיי")
...
def search(message):
    try:
        user_id = message.from_user.id
        m = bot.send_message(user_id, 'הכנס את שם השיר')
...
def callback(cbq):
    ...
    lb = list_builder.ListBuilder(names)
    message = bot.send_message(cbq.from_user.id, 'בחר את השיר', reply_markup=lb.get_keyboard())
